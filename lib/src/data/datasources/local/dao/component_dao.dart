...
class ComponentDao {
  ...
  Future<void> insertComponents(List<Component> components) async {
    ...
    await Future.forEach(components, (component) async {
      var foundProduct = await db.query(tableComponents, where: 'name = ?', whereArgs: [component.name]);

      if(foundProduct.isNotEmpty){
        batch.update(tableComponents, component.toJson(), where: 'name = ?', whereArgs: [component.name]);
      } else {
