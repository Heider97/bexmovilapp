...
class RawQueryDao {
  ...
  Future<RawQuery?> findRawQuery(int id, bool isSingle) async {
    final db = await _appDatabase.database;
    final rawQueryList = await db!.rawQuery(tableRawQueries, where: 'component_id = ?', whereArgs: [id]);
  ...
  Future<int> updateRawQuery(RawQuery rawQuery) {
    return _appDatabase.update(tableRawQueries, rawQuery.toJson(), 'id', rawQuery.id!);
  }
  ...
  Future<void> insertQueries(List<RawQuery> rawQueries) async {
    ...
    if (rawQueries.isNotEmpty) {
      await Future.forEach(rawQueries, (rawQuery) async {
        var d = await db.rawQuery(tableRawQueries, where: 'id = ?', whereArgs: [rawQuery.id]);
        var w = parseQueries(d);
        if (w.isEmpty) {
          batch.insert(tableRawQueries, rawQuery.toJson());
        } else {
          batch.update(tableRawQueries, rawQuery.toJson(), where: 'id = ?', whereArgs: [rawQuery.id]);
        }
      });
    }
